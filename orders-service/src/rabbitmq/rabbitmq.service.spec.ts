...
describe('RabbitMQService', () => {
  ...
  describe('onModuleInit', () => {
    ...
    it('deve configurar o exchange, filas e bindings', async () => {
      // Arrange
      // Captura o setup passado para createChannel
      let setupFn;
      ...
      // Act
      await service.onModuleInit();
      // Chama manualmente o setup para simular o comportamento real
      if (setupFn) {
  ...
  describe('publish', () => {
    ...
    it('deve publicar uma mensagem no exchange', async () => {
      // Arrange
    ...
    it('deve publicar mensagem de pagamento processado', async () => {
      // Arrange
  ...
  describe('consume', () => {
    ...
    it('deve registrar consumidor e processar mensagem', async () => {
      // Arrange
      const queue = 'order.created';
      const callback = jest.fn().mockResolvedValue(undefined);

      // Mockar o addSetup para executar imediatamente a função
    ...
    it('deve fazer ACK de mensagem processada com sucesso', async () => {
      // Arrange
      ...
      const mockMessage = {
        content: Buffer.from(JSON.stringify({ orderId: 1, status: 'PAID' })),
      };
    ...
    it('deve fazer NACK de mensagem com erro no processamento', async () => {
      // Arrange
      const queue = 'order.created';
    ...
    it('deve ignorar mensagens null', async () => {
      // Arrange
      const queue = 'order.created';
  ...
  describe('Getters', () => {
    it('deve retornar getOrderCreatedRoutingKey', () => {
      // Act
      const key = service.getOrderCreatedRoutingKey();

      // Assert
      expect(key).toBe('order.created');
    });

    it('deve retornar getPaymentProcessedQueue', () => {
      // Act
      const queue = service.getPaymentProcessedQueue();

      // Assert
      expect(queue).toBe('payment.processed');
    });
  });
});
